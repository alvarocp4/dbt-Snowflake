...
base as (
    select
        ...
        o.prospect_status,
        collate(trim(o.ds_acd), '')        as ds_acd,-- ABRVRECURSO (RE%) + join ACD
        collate(trim(o.segment_subtype), '') as segment_subtype, -- DESTIPOINST (COMUNIDAD)
        collate(trim(o.ds_origin), '') as ds_origin, -- DESORIGEN
    ...
    left join src_maestros_acds acd
        on upper(o.ds_acd) = upper(trim(acd.ds_acd))
    ...
    where
        ...
        -- Origen
        and upper(ds_origin) not like '%CAIXA%'
        and upper(ds_origin) not like '%MAPFRE%'
        and coalesce(o.ds_origin,'') <> 'VISITA TMK ANDORRA'
        -- Asignado
